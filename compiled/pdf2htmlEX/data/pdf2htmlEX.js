(function() {
    ...
    Viewer.prototype = {
        ...
        init_after_loading_content: function() {
			...
			if(!a){
				var sidebar=getparam(location.href,"sidebar");
				if(sidebar!='no'){
...
function g_init_onload(){
	var scale=getparam(location.href,"scale");
	var scale=parseFloat(scale);
	if(scale && !isNaN(scale)){
		pdf2htmlEX.defaultViewer.rescale(scale);
	}
}
if(window.addEventListener) window.addEventListener("load", g_init_onload, false);
else if (window.attachEvent) window.attachEvent("onload", g_init_onload);
